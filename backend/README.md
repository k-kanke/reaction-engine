...
+ ER diagrams for the current DB schema, regenerated by introspecting a live
local Postgres (config: `.tbls.yml`; `tbls` service in `compose.yaml`, also
behind the `tools` profile). Regenerate after changing migrations:
