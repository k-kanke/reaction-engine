...
// TranscriptStore persists transcript_chunk rows to the local/Cloud SQL
// `transcripts` table (migration 000001_initial_schema), per Phase 11 of
...
// EnsureSession upserts a minimal sessions row so transcripts (FK on
// session_id) can be inserted before a dedicated Session API exists.
// Mirrors internal/media.PGStore.EnsureSession.
func (s *TranscriptStore) EnsureSession(ctx context.Context, sessionID string) error {
...
func (s *TranscriptStore) InsertTranscriptChunk(ctx context.Context, chunk contract.TranscriptChunk) error {
	_, err := s.pool.Exec(ctx, `
